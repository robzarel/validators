...
const minLength: GetValidator<number, string> = (minCharCount = DEFAULT_MIN_CHAR_COUNT) => {
  if (minCharCount <= 0) {
    throw new Error(
  ...
  return async (value) =>
    value.length >= minCharCount ? null : `Количество символов не должно быть меньше ${minCharCount}`;
};
